...
public class TestProtocolSpecificWithMock {
	...
	protected static MockHandshakeMonitor monitor;
	...
	public void testStartServer() throws Exception {
		...
		monitor = new MockHandshakeMonitor();
		responder.addRPCPlugin(monitor.MockedHandshakeMonitor);
	}
	...
	@Test
	/**
	 * Construct and use a protocol whose "hello" method has an extra argument
	 * to check that schema is sent to parse request.
	 */
	...
	public class MockHandshakeMonitor {

		public RPCPlugin MockedHandshakeMonitor;

		private int handshakes;
		private HashSet<String> seenProtocols = new HashSet<>();

		public MockHandshakeMonitor() {
			this.MockedHandshakeMonitor = EasyMock.partialMockBuilder(RPCPlugin.class).addMockedMethod("serverConnecting", RPCContext.class)
					.createMock();
			mockServerConnecting();
			EasyMock.replay(this.MockedHandshakeMonitor);
		}
		...
		private void mockServerConnecting() {
			this.MockedHandshakeMonitor.serverConnecting(EasyMock.anyObject(RPCContext.class));
	...
	public static class InteropTest {
		...
		public static void main(String[] args) throws Exception {
			SocketServer server = new SocketServer(new SpecificResponder(Simple.class, new TestImpl()), new InetSocketAddress(0));
			server.start();
